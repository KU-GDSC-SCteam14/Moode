...
# 모델 로딩 코드를 여기에 배치 (서버 시작 시 한 번만 실행)
...
def post():
    ...
    # 여기에 모델을 사용하여 입력 처리 코드 배치
    top_4 = keyword_extraction(bert_model, kw_model, model, tokenizer, decoder_from_res, input_text)

    # 처리 결과를 JSON 형태로 반환
    return jsonify({'keywords': top_4})
